...
describe('Service Coverage Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.log('✅ Service test accounts funded');
    } catch (error) {
  ...
  afterAll(async () => {
    console.log('📊 Service Coverage Test Summary completed');
  });

  describe('Analytics Service Coverage', () => {
    test('getPlatformAnalytics - all timeframes', async () => {
      console.log('📊 Testing platform analytics with all timeframes...');
      ...
      for (const timeframe of timeframes) {
        ...
        console.log(`✅ ${timeframe} analytics: ${analytics.totalTransactions} transactions, ${analytics.activeAgents} agents`);
      }
    });

    test('getVolumeTimeSeries - all timeframes', async () => {
      console.log('📈 Testing volume time series...');
      ...
      for (const timeframe of timeframes) {
        ...
        console.log(`✅ ${timeframe} time series: ${timeSeries.length} data points`);
      }
    });

    test('getTopAgents - various limits', async () => {
      console.log('🏆 Testing top agents retrieval...');
      ...
      for (const limit of limits) {
        ...
        console.log(`✅ Top ${limit} agents: Retrieved ${topAgents.length} agents`);
      }
    });

    test('getAgentAnalytics - comprehensive agent data', async () => {
      console.log('📋 Testing agent-specific analytics...');
      ...
      console.log(`✅ Agent analytics: ${performance.totalJobs} jobs, ${performance.rating} rating`);
    });

    test('getNetworkHealth - network metrics', async () => {
      console.log('🌐 Testing network health metrics...');
      ...
      console.log(`✅ Network health: Block ${networkHealth.blockHeight}, ${networkHealth.averageBlockTime}s avg block time`);
    });
    ...
    test('generateReport - comprehensive reports', async () => {
      ...
      for (const timeframe of timeframes) {
        ...
        console.log(`✅ ${timeframe} report: ${reportWithAgents.summary.totalTransactions} transactions`);
      }
    });
    ...
    test('Analytics Service error handling', async () => {
      ...
      try {
        ...
        console.log('⚠️ Network health succeeded with invalid RPC');
      } catch (error) {
        expect(error).toBeDefined();
        expect(error instanceof Error).toBe(true);
        console.log('✅ Network health properly handles RPC errors');
      }
    });
  });
  ...
  describe('Compressed NFTs Service Coverage', () => {
    test('createCompressedNft - input validation', async () => {
      console.log('🌳 Testing compressed NFT input validation...');
      ...
      try {
        ...
        console.log('✅ Empty URI validation works');
      }
      ...
      try {
        ...
        console.log('✅ Negative seller fee validation works');
      }
      ...
      try {
        ...
        expect(error).toBeDefined();
        expect(error.message).toContain('must be between 0 and 10000');
        console.log('✅ Excessive seller fee validation works');
      }
      ...
      try {
        ...
        console.log('✅ Merkle tree validation works');
      }
    });
    ...
    test('Compressed NFT service constructor and properties', async () => {
      ...
      for (const commitment of commitments) {
        ...
        console.log(`✅ Service created with ${commitment} commitment`);
      }

      // Test default commitment
      const defaultService = new CompressedNftService(rpc, programId);
      expect(defaultService).toBeDefined();
      console.log('✅ Service created with default commitment');
    });
  });
  ...
  describe('Transaction Helpers Coverage', () => {
    ...
    test('PDA and instruction utilities', async () => {
      console.log('🔗 Testing PDA and instruction utilities...');
      ...
      console.log('✅ PDA and instruction utilities tested');
    });
  });

  describe('Client Service Integration Coverage', () => {
    test('Client lazy service loading', async () => {
      console.log('🔄 Testing client service lazy loading...');
    ...
    test('Client utility methods', async () => {
      ...
      console.log(`✅ Client utilities: Connected=${isConnected}, Balance=${balance.toFixed(6)} SOL`);
    });
    ...
    test('Client error handling', async () => {
      ...
      try {
        ...
        console.log('⚠️ Invalid address balance check succeeded (may be handled gracefully)');
      } catch (error) {
        expect(error).toBeDefined();
      ...
      try {
        ...
        console.log('⚠️ Zero airdrop succeeded (may be allowed)');
      } catch (error) {
        expect(error).toBeDefined();
        console.log('✅ Zero airdrop properly handled');
      }
    });
  });
  ...
  describe('Edge Cases and Error Conditions', () => {
    test('Service initialization edge cases', async () => {
      ...
      for (const commitment of commitments) {
        ...
        console.log(`✅ Services initialized with ${commitment} commitment`);
      }
    });
    ...
    test('Boundary value testing', async () => {
      ...
      console.log(`✅ Boundary testing: 0 agents=${zeroAgents.length}, 1000 agents=${excessiveAgents.length}`);
    });
