...
export interface IMarketplaceAnalytics {
  ...
  sellerStats: Array<{ seller: Address; listings: number; sales: number; volume: bigint }>;
}
...
export class MarketplaceService {
  ...
  async searchMarketplace(
    ...
    try {
      console.log('🔍 Performing advanced marketplace search with filters:', filters);
      ...
      return {
        ...
        searchMetadata: {
          ...
          filters,
          executionTime,
          relevanceScores: filters.searchQuery ? this.calculateRelevanceScores(paginatedListings, filters.searchQuery) : undefined,
        },
      };
    } catch (error) {
   ...
   */
  async getTrendingListings(limit: number = 20): Promise<IMarketplaceListing[]> {
    try {
      console.log('🔥 Getting trending marketplace listings');
  ...
  async getRecommendedListings(
    ...
    try {
      console.log(`🎯 Getting personalized recommendations for user: ${userAddress}`);
  ...
  async getMarketplaceAnalytics(): Promise<IMarketplaceAnalytics> {
    try {
      console.log('📊 Generating comprehensive marketplace analytics');
      ...
      const priceRanges = [
        ...
        { range: '10+ SOL', min: 10000000000n, max: BigInt(Number.MAX_SAFE_INTEGER) },
      ];
      ...
      const sellerMap = new Map<string, { listings: number; sales: number; volume: bigint }>();
      
      allListings.forEach(listing => {
        const seller = listing.seller;
        const current = sellerMap.get(seller) || { listings: 0, sales: 0, volume: 0n };
        current.listings++;
  ...
  private applyTextSearch(
    ...
    const searchFields = filters.searchFields || ['title', 'description', 'tags'];
  ...
  private calculateRelevanceScores(
    ...
    return listings.map(listing => {
      const searchableContent = [listing.id, listing.seller, listing.agentId].join(' ').toLowerCase();
  ...
  private calculatePopularityScore(listing: IMarketplaceListing): number {
    ...
    const timeFactor = Math.max(0, 1 - (Date.now() - listing.listedAt) / (30 * 24 * 60 * 60 * 1000));
  ...
  private getSellerReputation(seller: Address): number {
    ...
    const hash = seller.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  ...
  private extractCategoryFromListing(listing: IMarketplaceListing): string {
    ...
    const categories = ['AI Agents', 'Data Analysis', 'Content Creation', 'Trading Bots', 'NFT Tools'];
