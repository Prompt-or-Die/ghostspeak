...
export class ConfidentialTransferService {
  ...
  async createConfidentialAccount(
    ...
    try {
      console.log('🔐 Creating confidential token account');
  ...
  async configureAccount(
    ...
    try {
      console.log('⚙️ Configuring confidential transfers for account');
  ...
  async confidentialTransfer(
    ...
    try {
      console.log('🔄 Executing confidential transfer:', amount);
  ...
  async applyPendingBalance(
    ...
    try {
      ...
      console.error('❌ Failed to apply pending balance:', error);
  ...
  private validateProofs(proof: {
    ...
    return proof.equalityProof.length > 0 && 
   ...
   */
  private decryptBalance(encryptedBalance: string, _decryptionKey: string): string {
