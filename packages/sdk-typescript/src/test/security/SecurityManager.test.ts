...
describe('SecurityManager - Comprehensive Security Tests', () => {
  ...
  describe('CRITICAL-01: Authentication & Authorization', () => {
    ...
    test('should validate capabilities correctly', () => {
      ...
      expect(securityManager.hasCapability(capabilities, requiredRead)).toBe(true);
      expect(securityManager.hasCapability(capabilities, requiredWrite)).toBe(true);
      expect(securityManager.hasCapability(capabilities, requiredAdmin)).toBe(true);
      expect(securityManager.hasCapability(capabilities, invalidRequired)).toBe(false);
    });
  });
  ...
  describe('CRITICAL-02: Input Validation & Sanitization', () => {
    ...
    test('should detect and block XSS attacks', () => {
      const maliciousInputs = SecurityTestUtils.generateMaliciousInputs().xssPayloads;
    ...
    test('should detect and block SQL injection attempts', () => {
      const maliciousInputs = SecurityTestUtils.generateMaliciousInputs().sqlInjection;
    ...
    test('should detect and block command injection attempts', () => {
      const maliciousInputs = SecurityTestUtils.generateMaliciousInputs().commandInjection;
  ...
  describe('CRITICAL-03: Rate Limiting & DoS Protection', () => {
    ...
    test('should block requests exceeding rate limit', async () => {
      ...
      for (let i = 0; i < limit; i++) {
        await securityManager.checkRateLimit('test-client', requestInfo, limit, 60000);
      }
      
      // Next request should be blocked
      const result = await securityManager.checkRateLimit('test-client', requestInfo, limit, 60000);
  ...
  describe('CRITICAL-05: Cryptographic Security', () => {
    ...
    test('should reject invalid buffer sizes', () => {
      ...
      expect(() => new SecureBuffer(2 * 1024 * 1024)).toThrow('Invalid buffer size'); // > 1MB
    });
  });
  ...
  describe('PERFORMANCE: Security Operation Performance', () => {
    ...
    test('should handle rate limiting efficiently', async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        await securityManager.checkRateLimit(`client-${i}`, requestInfo, 10, 60000);
      }
...
describe('Security Integration Tests', () => {
  test('should handle complete security workflow', async () => {
    ...
    // 3. Check rate limit
    const requestInfo = SecurityTestUtils.createTestRequestInfo();
    const rateLimitResult = await manager.checkRateLimit('integration-client', requestInfo);
    ...
    const signatureValid = await manager.verifySignature(message, signature, keypair.publicKey);
