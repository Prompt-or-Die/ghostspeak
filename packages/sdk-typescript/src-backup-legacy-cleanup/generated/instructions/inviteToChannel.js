...
export function getInviteToChannelInstructionDataEncoder() {
...
export function getInviteToChannelInstructionDataDecoder() {
...
export function getInviteToChannelInstructionDataCodec() {
  return combineCodec(
    getInviteToChannelInstructionDataEncoder(),
    getInviteToChannelInstructionDataDecoder()
  );
}
export async function getInviteToChannelInstructionAsync(input, config) {
  // Program address.
  ...
  const instruction = {
    ...
    programAddress,
    data: getInviteToChannelInstructionDataEncoder().encode(args),
  };
  return instruction;
}
export function getInviteToChannelInstruction(input, config) {
  // Program address.
  const programAddress = config?.programAddress ?? POD_COM_PROGRAM_ADDRESS;
  // Original accounts.
  const originalAccounts = {
...
export function parseInviteToChannelInstruction(instruction) {
  if (instruction.accounts.length < 6) {
    throw new Error(
      'InviteToChannel instruction: Insufficient accounts provided. Expected at least required accounts for channel invitation'
    );
  }
  let accountIndex = 0;
...
//# sourceMappingURL=inviteToChannel.js.map
