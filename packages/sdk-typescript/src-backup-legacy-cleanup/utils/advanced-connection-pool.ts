...
export class AdvancedConnectionPool {
  ...
  private selectFastestResponseTime(endpoints: string[]): string {
    return endpoints.reduce((optimal, current) => {
      ...
      return currentMetrics.averageResponseTime <
        optimalMetrics.averageResponseTime
        ? current
        : optimal;
    });
  }
  ...
  private selectWeighted(endpoints: string[]): string {
    ...
    return endpoints[(endpoints as any).length - 1];
  }
  ...
  private async getConnectionForEndpoint(
    ...
    const availableConnection = connections.find(
      (conn) => conn.isHealthy && Date.now() - conn.lastUsed < 300000 // 5 minutes idle timeout
    );
  ...
  private async createConnection(
    ...
    try {
      ...
      throw new NetworkError(
        `Failed to create connection to ${endpoint}: ${error}`
      );
    }
  }
   ...
   */
  private recordFailure(endpoint: string, _error: Error): void {
    const metrics = this.endpointMetrics.get(endpoint)!;
    const circuitBreaker = this.circuitBreakers.get(endpoint)!;
  ...
  getStats(): PoolStats {
    ...
    const activeRequests = allMetrics.reduce(
      (sum, m) => sum + m.activeConnections,
      0
    );

    const totalResponseTime = allMetrics.reduce(
      (sum, m) => sum + m.averageResponseTime * m.successfulRequests,
      0
    );
  ...
  async addEndpoint(endpoint: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `Failed to initialize connection to new endpoint ${endpoint}:`,
        error
      );
    }
  }
