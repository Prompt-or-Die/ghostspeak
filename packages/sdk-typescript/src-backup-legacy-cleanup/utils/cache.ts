...
export class LRUCache<K = string, V = unknown> {
  ...
  constructor(maxSize: number = 100, __ttl: number = 300000) {
...
export class Cache<K, V> {
  ...
  ): Array<{ key: K; value: V; expiresIn: number }> {
    const now = Date.now();
...
export class AccountCache extends LRUCache {
  ...
  static keys = {
    ...
    typedAccount: (type: string, address: string) =>
  ...
  getProgramAccounts(accountType: string, filters: any[]): any[] | undefined {
    return this.get(AccountCache.keys.programAccounts(accountType, filters)) as
      | any[]
      | undefined;
  }
  ...
  invalidateByType(_accountType: string): number {
    const count = 0;
    // Implementation would scan keys and remove matching patterns
    return count;
  }

  invalidateByProgram(_programId: string): number {
    const count = 0;
    // Implementation would scan keys and remove matching patterns
    return count;
  }

  invalidate(_predicate: (key: string) => boolean): number {
    const count = 0;
    // Implementation would scan keys and remove matching patterns
    return count;
  }
