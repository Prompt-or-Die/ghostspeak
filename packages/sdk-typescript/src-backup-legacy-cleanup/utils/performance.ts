    ...
    for (let i = 0; i < buckets; i++) {
      ...
      const averageDuration =
        bucketMetrics.length > 0
          ? bucketMetrics.reduce((sum, m) => sum + m.duration, 0) /
      ...
      trends.unshift({
        ...
        averageDuration,
        successRate:
          bucketMetrics.length > 0 ? successes / bucketMetrics.length : 0,
      });
    }
...
export class ConnectionPool {
  ...
  constructor(maxConnections: number = 10, maxIdleTimeMs: number = 300000) {
    // 5 min idle timeout
...
export class PerformanceBenchmark {
  ...
  static compare(
    ...
    const speedImprovement =
      (benchmark2.opsPerSecond - benchmark1.opsPerSecond) /
      benchmark1.opsPerSecond;
    const memoryImprovement =
      (benchmark1.memoryStats.growth - benchmark2.memoryStats.growth) /
      benchmark1.memoryStats.growth;

    const __speedWinner =
      speedImprovement > 0 ? benchmark2.name : benchmark1.name;
    const __memoryWinner =
      memoryImprovement > 0 ? benchmark2.name : benchmark1.name;

    const winner =
      speedImprovement > 0 && memoryImprovement > 0
        ? benchmark2.name
        : speedImprovement < 0 && memoryImprovement < 0
          ? benchmark1.name
          : speedImprovement > Math.abs(memoryImprovement)
            ? benchmark2.name
            : benchmark1.name;

    const summary =
      `${benchmark2.name} vs ${benchmark1.name}: ` +
