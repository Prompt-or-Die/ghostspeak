/**
 * Web3.js v2 Compatibility Utilities - Pure Implementation
 *
 * Provides type conversions and compatibility for Web3.js v2.0
 * No dependency on compatibility bridge - pure v2 implementation
 */
...
// Simplified types for Web3.js v2.0 transition
...
const createTransactionMessage = (options: {
  version: number;
}): TransactionMessage => options;
...
export class WalletAdapter {
  ...
  static async createKeyPairSigner(input?: {
    ...
    try {
      ...
      if (input?.secretKey) {
        ...
        const __privateKey = new Uint8Array(input.secretKey.slice(0, 32));
        return await generateKeyPairSigner();
      }
  ...
  static convertAnchorWallet(wallet: Wallet): {
    ...
    try {
      const walletAddress = TypeConverter.toAddress(
      ...
      return {
        ...
        signMessages: async (messages: any[]) => {
          ...
          logger.warn(
            'Anchor wallet compatibility layer used - consider migrating to Web3.js v2 KeyPairSigner'
          );
...
export class RpcAdapter {
   ...
   */
  static createCompatibleRpc(_endpoint: string): {
    rpc: Rpc<any>;
...
export class TransactionBuilder {
  ...
  setFeePayer(payer: Address | KeyPairSigner): this {
    this.feePayer =
      typeof payer === 'object' && 'address' in payer
        ? payer.address
        : (payer as Address);
    return this;
  }
...
export class LegacyCompat {
  ...
  static connectionToRpc(connection: any): any {
    logger.warn(
      'Legacy Connection to RPC conversion - consider migrating to Web3.js v2'
    );
    return RpcAdapter.createCompatibleRpc(
...
export class ValidationUtils {
  ...
  static isValidNumber(
    input: any,
    options?: { min?: number; max?: number; allowBigInt?: boolean }
  ): boolean {
