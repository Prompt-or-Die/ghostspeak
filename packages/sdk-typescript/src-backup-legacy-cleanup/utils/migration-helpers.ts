 ...
 */
export function wrapRpcResponse<T>(
  value: T,
  slot?: number
): LegacyRpcResponse<T> {
  return {
...
export async function checkMigrationStatus(): Promise<MigrationStatus> {
  try {
    ...
    const __slot = await testRpc.getSlot().send();
...
export function formatMigrationError(originalError: any): Error {
  ...
  if (message.includes('Address')) {
    return new Error(
      `Migration Error: Replace Address with Address. Original: ${message}`
    );
  }

  if (message.includes('Rpc<any>')) {
    return new Error(
      `Migration Error: Replace Rpc<any> with Rpc. Original: ${message}`
    );
  }

  if (message.includes('Signer')) {
    return new Error(
      `Migration Error: Replace Signer with KeyPairSigner. Original: ${message}`
    );
  }

  if (message.includes('toBase58')) {
    return new Error(
      `Migration Error: Address is already a string in v2.0. Original: ${message}`
    );
  }
...
export const MIGRATION_CONFIG = {
  ...
  REMAINING_FILES: ['services/analytics.ts', 'services/discovery.ts'],
} as const;

/**
 * Quick migration validator
 */
export function validateMigration(_fileName: string): {
  isValid: boolean;
  issues: string[];
} {
  const issues: string[] = [];
