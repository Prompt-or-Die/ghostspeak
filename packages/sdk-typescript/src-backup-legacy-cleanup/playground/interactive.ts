    ...
    switch (action?.toLowerCase()) {
      ...
      case 'set':
        ...
        if (key === 'debugMode') {
          ...
          logger.updateConfig({
            logLevel: this.state.config.debugMode ? 0 : 1,
          });
  ...
  private handleValidation(args: string[]): void {
    ...
    switch (type.toLowerCase()) {
      case 'address':
        ...
        console.log(
          `Address validation: ${isValidAddress ? '✅ Valid' : '❌ Invalid'}`
        );
        if (isValidAddress) {
      ...
      case 'number':
        ...
        console.log(
          `Number validation: ${isValidNumber ? '✅ Valid' : '❌ Invalid'}`
        );
        if (isValidNumber) {
    ...
    try {
      switch (type.toLowerCase()) {
        ...
        case 'address':
          ...
          console.log(
            `Formatted: ${DevUtils.formatAddress(addressValue.toString())}`
          );
          break;
