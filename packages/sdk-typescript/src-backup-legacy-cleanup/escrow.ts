...
export class EscrowService extends BaseService {
  ...
  private convertEscrowAccountFromProgram(account: any): EscrowAccount {
    return {
      ...
      amount: account.balance?.toNumber() || 0,
   ...
   */
  async release(_escrowId: string): Promise<{ signature: string }> {
    ...
    throw new Error(
      'Release functionality requires channel address and amount. Use withdrawEscrow directly.'
    );
  }
