...
export class AgentService extends BaseService {
    ...
    return retry(async () => {
      ...
      const tx = await (
        program.methods as unknown as {
          // eslint-disable-next-line no-unused-vars
          updateAgent: (
            _capabilities: anchor.BN | null,
            _metadataUri: string | null
          ) => {
            // eslint-disable-next-line no-unused-vars
            accounts: (_accounts: {
              agentAccount: Address;
    ...
    try {
      ...
      if (this.program) {
        ...
        const readOnlyWallet = {
          publicKey:
            tempKeyPairSigner.address as unknown as anchor.web3.PublicKey,
      ...
      const agentAccount = (
        program.account as unknown as {
          agentAccount: { fetch: (_address: Address) => Promise<unknown> };
        }
      ).agentAccount;
  ...
  async getAllAgents(limit: number = 100): Promise<AgentAccount[]> {
    try {
      ...
      const agentAccount = (
        program.account as unknown as {
          agentAccount: { all: () => Promise<unknown[]> };
        }
      ).agentAccount;
      ...
      return accounts
        .slice(0, limit)
        .map((acc: unknown, index: number, array: unknown[]) => {
          // Type assertion for the account structure
          ...
          return {
            pubkey: typedAcc.publicKey,
            capabilities: typedAcc.account.capabilities.toNumber(),
            metadataUri: typedAcc.account.metadataUri,
            reputation: typedAcc.account.reputation?.toNumber() || 0,
            lastUpdated: getAccountLastUpdated(typedAcc.account),
            invitesSent: 0,
            lastInviteAt: 0,
            bump: typedAcc.account.bump,
          };
        });
    } catch (error: unknown) {
      const errorMessage =
