...
// Security types defined locally
...
export class SecurityManager {
  ...
  async validateRequest<T>(
    ...
    try {
      // Rate limiting check
      if (metadata?.ip && !(await this.checkRateLimit(metadata.ip))) {
        return {
          valid: false,
  ...
  getSecurityStatus(): 'healthy' | 'warning' | 'critical' {
    const activeConnections = this.requestCounts.size;

    if (activeConnections > 1000) return 'critical';
    if (activeConnections > 500) return 'warning';
    return 'healthy';
  }
  ...
  async verifySignature(
    ...
    publicKey: PublicKey | Address | Address | Address
  ): Promise<boolean> {
    try {
  ...
  validateSession(sessionId: string): SessionValidationResult {
    ...
    const now = Date.now();
    const isExpired =
      now - session.lastActivity > this.config.validation.maxInputLength;
  ...
  generateSecurityReport(): SecurityReport {
    ...
    const rateLimitedClients = Array.from(this.rateLimitStore.entries()).filter(
      ([_, window]) => now - window.firstRequest < windowMs
    ).length;
    ...
    return {
      ...
      metrics: {
        ...
        criticalEvents: recentEvents.filter((e) => e.severity === 'critical')
          .length,
        highSeverityEvents: recentEvents.filter((e) => e.severity === 'high')
          .length,
      },
      recentIncidents: recentEvents.filter(
  ...
  // Fix validation result type
  private sanitizeAndValidate<T>(
    input: unknown,
    config: SanitizationConfig
  ): ValidationResult<T> {
    try {
      // Perform sanitization
      const sanitized = this.performSanitization(input, config);
  ...
  // Simple encryption without external deps
  private performEncryption(
    ...
    // Simple XOR encryption for now (replace with proper crypto later)
    const dataBytes = data.getData();
    const encrypted = new Uint8Array(dataBytes.length);
    for (let i = 0; i < dataBytes.length; i++) {
      encrypted[i] =
        dataBytes[i] ^ keyPair.secretKey[i % keyPair.secretKey.length];
    }
    return encrypted;
  }

  // Fix undefined checks
  private validateKeyPair(keyPair: EncryptionKeyPair | undefined): boolean {
    return Boolean(
      keyPair?.publicKey &&
        keyPair?.secretKey &&
