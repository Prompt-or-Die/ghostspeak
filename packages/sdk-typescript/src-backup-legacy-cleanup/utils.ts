...
import { PROGRAM_ID, MessageType } from './types';
...
async function deriveCompatiblePDA(
  seeds: Uint8Array[],
  programId: Address
): Promise<[Address, number]> {
  ...
  for (let bump = 255; bump >= 0; bump--) {
    ...
    const isValid =
      hashArray.some((byte, index) => byte !== hashArray[0]) &&
      hashArray[0] !== 0;
...
export function generateMessageId(): string {
  ...
  const counter = ((globalThis as any).__msgIdCounter =
    ((globalThis as any).__msgIdCounter || 0) + 1);
  ...
  if (typeof crypto !== 'undefined' && crypto.getRandomValues) {
    ...
    const randomStr = Array.from(array, (byte) => byte.toString(36))
      .join('')
      .slice(0, 6);
 ...
 */
export function isValidAddress(pubkey: Address | string): boolean {
  try {
