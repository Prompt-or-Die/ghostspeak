...
export class PodComClient {
  ...
  constructor(config: PodClientConfig) {
    ...
    const programIdStr =
      typeof programId === 'string' ? programId : address(programId as string);
    ...
    this.jitoBundles = new JitoBundlesService(
      endpoint,
      programIdStr,
      commitment
    );
    this.sessionKeys = new SessionKeysService(
      endpoint,
      programIdStr,
      commitment
    );
   ...
   */
  async getAllChannels(limit: number = 50): Promise<ChannelAccount[]> {
  ...
  async broadcastMessage(
    ...
    return this.channels.broadcastMessage(wallet, messageOptions);
  }
  ...
  async discoverAgents(
    ...
    if (Array.isArray(searchParams.capabilities)) {
      agentFilters.capabilities = searchParams.capabilities.map((cap) =>
        typeof cap === 'string' ? parseInt(cap) || 0 : cap
      );
    }
    if (searchParams.limit !== undefined) {
      agentFilters.limit = searchParams.limit;
    }
  ...
  async createEscrow(escrowData: {
    ...
    const createOptions: any = {
      ...
      _amount: escrowData.amount,
      conditions: escrowData.conditions,
    };
    if (escrowData.timeoutHours !== undefined) {
      createOptions.expiresIn = escrowData.timeoutHours * 3600;
    }
    ...
    return {
      escrow: {
        ...
        status: 'pending',
        createdAt: Date.now(),
        expiresAt: escrowData.timeoutHours
          ? Date.now() + escrowData.timeoutHours * 3600000
          : undefined,
      },
