...
export class JitoBundlesService extends BaseService {
      ...
      const signatures = await Promise.all(
        signers.map(async (signer, index) => {
          ...
          for (let i = 0; i < 88; i++) {
            ...
            const index =
              hashArray[i % hashArray?.length] % base58Chars?.length;
  ...
  async getBundleStatus(bundleId: string): Promise<BundleResult> {
    try {
      const response = await fetch(
        `${this.jitoRpcUrl}/status?bundle=${bundleId}`
      );
  ...
  private async createTipTransaction(
    ...
    const tipAccount = address(this.getNextTipAccount());
  ...
  private async submitToJito(transactions: any[]): Promise<BundleResult> {
    try {
      ...
      if (data.error) {
        throw new Error(
          `Jito error: ${data.error.message || JSON.stringify(data.error)}`
        );
      }

      // Extract signatures from transactions - these would come from Jito response
      const signatures = await this.generateBundleSignatures(
        data.result,
        transactions
      );
  ...
  async estimateBundleCost(
    ...
    const priorityFees =
      (priorityFee * computeUnits * transactionCount) / 1_000_000; // Convert micro-lamports to lamports
  ...
  private async generateBundleSignatures(
    bundleId: string,
    transactions: any[]
  ): Promise<string[]> {
    try {
  ...
  private getNextTipAccount(): Address {
    ...
    const currentIndex =
      ((globalThis as any).__jitoTipIndex || 0) % this.JITO_TIP_ACCOUNTS.length;
