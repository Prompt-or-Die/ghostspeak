...
export class SessionKeysService extends BaseService {
  ...
  private async sendTransactionWithAnchor(
    methodName: string,
    methodArgs: any[],
    accounts: any,
    ...
    let txBuilder = (program.methods as any)
  ...
  async createSessionKey(config: SessionKeyConfig): Promise<SessionToken> {
    try {
      ...
      const signature = await this.sendTransactionWithAnchor(
        'createSession',
        [
          new BN(config.expiryTime),
          config.maxUses ? new BN(config.maxUses) : null,
        ],
        {
    ...
    try {
      ...
      for (const instruction of instructions) {
        if (!this.isInstructionAllowed(instruction, session.config)) {
          throw new Error(
            `Instruction not allowed for this session: ${instruction.programAddress}`
          );
        }
      }
  ...
  private isInstructionAllowed(
    ...
    const programAllowed = config.targetPrograms.some((program) => {
      const programAddr =
        typeof program === 'string' ? address(program) : program;
      const instructionProgramAddr =
    ...
    const allowedInstruction = IDL.instructions.find((ix) =>
      Buffer.from(ix.discriminator).equals(instructionDiscriminator)
    );
