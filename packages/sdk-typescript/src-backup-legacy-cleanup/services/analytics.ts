...
import type { Address, Rpc, SolanaRpcApi } from '@solana/rpc';
// Web3.js v2 imports (modern API)
...
import { lamportsToSol, formatBytes, getCapabilityNames } from '../utils.js';
...
export class AnalyticsService extends BaseService {
  ...
  async getAccountMetrics(): Promise<AccountMetrics> {
    try {
      ...
      for (const account of accounts) {
        const accountType = this.detectAccountType(account.account.data);
        accountsByType[accountType] = (accountsByType[accountType] || 0) + 1;

        if (this.isAccountActive(account.account)) {
          activeAccounts++;
        }
      }
      ...
      throw new Error(
        `Failed to get account metrics: ${(error as Error).message}`
      );
    }
  }
  ...
  async getNetworkMetrics(): Promise<NetworkMetrics> {
    try {
      ...
      return {
        transactionCount: 0, // Would need block parsing
        averageTransactionTime: 0, // Would need transaction timing
        networkTps: 0, // Would need TPS calculation
      };
    } catch (error) {
      throw new Error(
        `Failed to get network metrics: ${(error as Error).message}`
      );
    }
  }

  async getAnalytics(): Promise<AnalyticsData> {
    const [accountMetrics, networkMetrics] = await Promise.all([
  ...
  private detectAccountType(data: Buffer): string {
    // Simple account type detection based on data structure
    if (data.length < 8) return 'unknown';

    const discriminator = data.subarray(0, 8);
    // Would map discriminators to account types
    return 'generic';
  }

  private isAccountActive(account: any): boolean {
    // Simple activity check - would be more sophisticated in production
      ...
      try {
        ...
        agentData.forEach((agent) => {
          ...
          capabilities.forEach((cap) => {
            capabilityDistribution[cap] =
              (capabilityDistribution[cap] || 0) + 1;
          });
        });
  ...
  async getMessageAnalytics(limit: number = 1000): Promise<MessageAnalytics> {
    try {
      ...
      const messageAccounts = await this.getProgramAccounts(
        'messageAccount',
        [],
      ...
      for (const message of messageData) {
        try {
          ...
          const messageStatus =
            ...
            message.status !== null &&
            !Object.values(MessageStatus).includes(
              message.status as MessageStatus
            )
              ? this.convertMessageStatusFromProgram(
          ...
          messagesByType[messageTypeStr] =
            (messagesByType[messageTypeStr] || 0) + 1;
      ...
      const averageMessageSize =
        totalMessages > 0 ? totalMessageSize / totalMessages : 0;
      ...
      return {
        ...
        recentMessages: messageData.slice(0, 10).map((msg) => {
          ...
          const status =
            ...
            msg.status !== null &&
            !Object.values(MessageStatus).includes(msg.status as MessageStatus)
              ? this.convertMessageStatusFromProgram(
                  msg.status as Record<string, unknown>
                )
              : (msg.status as MessageStatus) || MessageStatus.PENDING;

          return {
            pubkey: address(
              msg.pubkey?.toString() || '11111111111111111111111111111112'
            ),
            sender: msg.sender || address('11111111111111111111111111111112'),
            recipient:
              msg.recipient || address('11111111111111111111111111111112'),
            payload: msg.payload || '',
      ...
      throw new Error(
        `Failed to get message analytics: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private calculateMessagesPerDay(messageData: any[]): number {
    ...
    const recentMessages = messageData.filter((msg) => {
      const timestamp =
        (msg.timestamp?.toNumber() || msg.createdAt?.toNumber() || 0) * 1000;
    ...
    return this.getCachedOrFetch(
      ...
      async () => {
        try {
          ...
          const channelAccounts = await this.getProgramAccounts(
            'channelAccount',
            [],
            { limit }
          );
          ...
          const channelData = await this.processAccounts<ChannelAccount>(
            ...
            (decoded, account) => ({
              ...
              memberCount:
                decoded.currentParticipants || decoded.memberCount || 0,
              currentParticipants:
                decoded.currentParticipants || decoded.memberCount || 0,
              maxParticipants:
                decoded.maxParticipants || decoded.maxMembers || 0,
              participantCount:
                decoded.currentParticipants || decoded.memberCount || 0,
    ...
    return this.getCachedOrFetch(
      ...
      async () => {
        try {
          ...
          const [performanceData, escrowData, messageData, agentData] =
            await Promise.all([
          ...
          const escrowAccounts = await this.processAccounts(
            escrowData,
            'escrowAccount'
          );
          const totalValueLocked: number = escrowAccounts.reduce(
            (sum: number, acc: any): number => {
          ...
          const recentMessages = await this.processAccounts(
            messageData,
            'messageAccount'
          );
          const messageVolume24h = recentMessages.filter(
            (msg) =>
              (msg.timestamp?.toNumber() || 0) * 1000 > twentyFourHoursAgo
          ).length;

          const recentAgents = await this.processAccounts(
            agentData,
            'agentAccount'
          );
          const activeAgents24h = recentAgents.filter(
            (agent) =>
  ...
  private async getNetworkPerformanceData(): Promise<{
    ...
    try {
      return await RetryUtils.analytics(async () => {
        ...
        const avgTps =
          performanceSamples.reduce((sum: number, sample: any) => {
            return sum + sample.numTransactions / sample.samplePeriodSecs;
          }, 0) / totalSamples;

        const totalTransactions = performanceSamples.reduce(
  ...
  private calculatePeakUsageHours(messages: any[]): number[] {
    ...
    messages.forEach((msg) => {
      const timestamp =
        (msg.timestamp?.toNumber() || msg.createdAt?.toNumber() || Date.now()) *
        1000;
  ...
  private extractNumber(
    value: { toNumber(): number } | number | undefined | null
  ): number {
  ...
  private convertMessageTypeFromProgram(
    programType: Record<string, unknown>
  ): MessageType {
    ...
    return this.getCachedOrFetch(
      ...
      async () => {
        try {
          ...
          if (!agentAccount) {
            throw new Error(
              `Agent account not found: ${agentAddress.toString()}`
            );
          }

          const agentData = this.decodeAccountData(
            'agentAccount',
            agentAccount.account.data
          );
          ...
          const sentMessages = await this.getProgramAccounts('messageAccount', [
            sentMessagesFilter,
          ]);
          ...
          const receivedMessages = await this.getProgramAccounts(
            'messageAccount',
            [receivedMessagesFilter]
          );
          ...
          const sentMessageData = await this.processAccounts(
            sentMessages,
            'messageAccount'
          );
          const receivedMessageData = await this.processAccounts(
            receivedMessages,
            'messageAccount'
          );
          ...
          sentMessageData.forEach((sent) => {
            const relatedReceived = receivedMessageData.find(
              (received) =>
                Math.abs(
                  (sent.timestamp?.toNumber() || 0) -
                    (received.timestamp?.toNumber() || 0)
                ) < 3600 // Within 1 hour
            );
            if (relatedReceived) {
              responseTimes.push(
                Math.abs(
                  (sent.timestamp?.toNumber() || 0) -
                    (relatedReceived.timestamp?.toNumber() || 0)
                )
              );
            }
          });
          ...
          const successfulMessages = sentMessageData.filter(
            (msg) =>
              this.convertMessageStatusFromProgram(msg.status || {}) ===
              MessageStatus.DELIVERED
          ).length;
          const successRate =
            messagesSent > 0 ? successfulMessages / messagesSent : 0;
          ...
          throw ErrorHandler.classify(
            error,
            `getAgentMetrics(${agentAddress.toString()})`
          );
        }
      },
      this.analyticsCache
  ...
  async getMessageMetrics(
    ...
    try {
      ...
      for (const account of messageAccounts) {
        try {
          const messageData = this.program.coder.accounts.decode(
            'messageAccount',
      ...
      throw new Error(
        `Failed to get message metrics: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getChannelMetrics(channelAddress?: Address): Promise<ChannelMetrics> {
    try {
      ...
      for (const account of channelAccounts) {
        try {
          const channelData = this.program.coder.accounts.decode(
            'channelAccount',
            account.account.data as Buffer
          );
          activeChannels++;
      ...
      throw new Error(
        `Failed to get channel metrics: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (performanceSamples && performanceSamples.length > 0) {
          ...
          const totalTransactions = performanceSamples.reduce(
            (sum: number, sample: any) => sum + sample.numTransactions,
            0
          );
          const totalSlots = performanceSamples.reduce(
            (sum: number, sample: any) => sum + sample.numSlots,
            0
          );

          successRate =
            totalSlots > 0 ? Math.min(totalTransactions / totalSlots, 1) : 0.98;

          // Update confirmation time from real data
          avgConfirmationTime =
            (performanceSamples.reduce(
              (sum: number, sample: any) => sum + sample.samplePeriodSecs,
              0
            ) /
        ...
        const recentBlockhash = await (this.rpc as any)
          .getLatestBlockhash()
          .send();
        if (recentBlockhash) {
          // Fee estimation based on recent activity - simplified calculation
          avgTransactionFee = Math.max(
            5000,
            Math.min(50000, performanceData.averageTps * 2)
          );
        }
      } catch (error) {
        console.warn(
          'Failed to get detailed performance metrics, using defaults:',
          error
        );
      }
  ...
  private calculateReputation(metrics: {
    ...
    const { messagesSent, messagesReceived, averageResponseTime, successRate } =
      metrics;
  ...
  async getAgentStats(
    ...
    try {
      ...
      throw new Error(
        `Failed to get agent stats: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
