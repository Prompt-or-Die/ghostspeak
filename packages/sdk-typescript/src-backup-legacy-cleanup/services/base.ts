import { Web3Bridge, createWeb3Bridge } from '../compatibility/web3-bridge.js';
...
import { AccountCache, AnalyticsCache } from '../utils/cache.js';
import { ACCOUNT_SIZES, AccountFilters } from '../utils/account-sizes.js';
...
export class BaseService {
  ...
  async getProgramAccounts(
    ...
    // Create cache key
    const cacheKey = this.createAccountsCacheKey(
      accountType,
      additionalFilters,
      limit
    );
    ...
    if (useCache) {
      const cached = this.accountCache.get(cacheKey) as
        | SolanaAccountInfo[]
        | undefined;
      if (cached) {
        return cached;
    ...
    try {
      ...
      const filters = AccountFilters.createAccountFilters(
        accountType,
        additionalFilters
      );
      ...
      const result = await RetryUtils.rpcCall(async () => {
        ...
        if (this.rpc && typeof this.rpc.getProgramAccounts === 'function') {
          const __response = await this.rpc
            .getProgramAccounts(this.programId, { filters })
            .send();
      ...
      const accounts: SolanaAccountInfo[] = (result.value || result).map(
        (account: any) => ({
          pubkey: account.pubkey.toString(),
          account: {
            data: new Uint8Array(
              account.account.data[0] === 'string'
            ...
            rentEpoch: account.account.rentEpoch
              ? BigInt(account.account.rentEpoch)
              : undefined,
          },
        })
      );
  ...
  async getMultipleAccountsInfo(
    ...
    try {
      ...
      for (let i = 0; i < addresses.length; i += batchSize) {
        ...
        if (useCache) {
          const cached = this.accountCache.get(batchCacheKey) as
            | Array<SolanaAccountInfo | null>
            | undefined;
          if (cached) {
            batchResults = cached;
          } else {
  ...
  async getAccountInfo(
    ...
    const { useCache = this.enableCaching, __cacheTtl = 30000 } = options;
  ...
  protected async processAccounts<T>(
    ...
    for (const account of accounts) {
      try {
        const decoded = this.decodeAccountData(
          accountType,
          account.account.data
        );
  ...
  protected invalidateCache(pattern: {
    ...
    if (pattern.address) {
      count += this.accountCache.invalidate((key) =>
        key.includes(pattern.address!)
      );
    }
  ...
  private async fetchAccountsBatch(
    addresses: Address[]
  ): Promise<Array<SolanaAccountInfo | null>> {
    return RetryUtils.rpcCall(async () => {
      ...
      try {
        const __response = await (this.rpc as any)
  ...
  private updateConnectionHealth(success: boolean, latency: number): void {
    ...
    const totalCalls =
      this.connectionHealth.successCount + this.connectionHealth.errorCount;
    const successRate =
      totalCalls > 0 ? this.connectionHealth.successCount / totalCalls : 1;
  ...
  private startHealthMonitoring(): void {
    this.healthCheckInterval = setInterval(async () => {
      try {
        const __startTime = Date.now();
        const __response = await (this.rpc as any)
          .getSlot({ commitment: this.commitment })
          .send();
        this.updateConnectionHealth(true, Date.now() - startTime);
      } catch (error) {
        this.updateConnectionHealth(false, 0);
  ...
  public async getCurrentSlot(): Promise<bigint> {
    try {
      if (this.rpc && typeof (this.rpc as any).getSlot === 'function') {
        const slot = await (this.rpc as any)
          .getSlot({ commitment: this.commitment })
          .send();
        return BigInt(slot);
      } else {
  ...
  async getRecentPerformanceSamples(limit: number = 10): Promise<any[]> {
    try {
      if (
        this.rpc &&
        typeof (this.rpc as any).getRecentPerformanceSamples === 'function'
      ) {
        return await (this.rpc as any)
