...
import {} from '../utils/error-handling.js';
...
export class PerformanceBenchmarkService {
  ...
  private async createDiscoveryBenchmarks(): Promise<BenchmarkSuite> {
    ...
    return {
      ...
      description:
        'Benchmarks for agent discovery and recommendation algorithms',
      benchmarks: [
        {
          name: 'Agent Search Performance',
          operation: () =>
            this.discoveryService!.searchAgents({
        ...
        },
        {
          ...
          operation: () =>
            this.discoveryService!.getAgentRecommendations(
              address('test_agent'),
              10
            ),
          expectedOps: 20,
        ...
        },
        {
          ...
          operation: () =>
            this.discoveryService!.searchChannels({
        ...
        },
        {
          ...
          operation: async () => {
            ...
            const agents = await this.discoveryService!.searchAgents({
              limit: 100,
            });
  ...
  private async createChannelBenchmarks(): Promise<BenchmarkSuite> {
    if (!this.channelService) {
      return {
        name: 'Channel',
        description: 'Channel service not available',
        benchmarks: [],
      };
    }
        ...
        },
        {
          ...
          operation: async () => {
            ...
            if (channels.length > 0) {
              return this?.channelService!.getChannelParticipants(
                channels[0].pubkey
              );
            }
            return [];
  ...
  private async createZKCompressionBenchmarks(): Promise<BenchmarkSuite> {
    if (!this.zkCompressionService) {
      return {
        name: 'ZK Compression',
        description: 'ZK compression service not available',
        benchmarks: [],
      };
    }
      ...
      benchmarks: [
        {
          ...
          operation: () =>
            this.zkCompressionService!.compressMessage(
              'test_channel',
        ...
        },
        {
          ...
          operation: () =>
            Promise.resolve(
              this.zkCompressionService!.getStats('test_channel')
            ),
          expectedOps: 50,
          timeout: 1000,
        },
  ...
  private async runBenchmarkSuite(suite: BenchmarkSuite): Promise<{
    ...
    for (const benchmark of suite.benchmarks) {
      try {
        ...
        const passed = benchmark.expectedOps
          ? result.opsPerSecond >= benchmark.expectedOps
          : true;
        const score = this.calculateBenchmarkScore(
          result,
          benchmark.expectedOps
        );
    ...
    const overallScore =
      benchmarkResults.length > 0 ? totalScore / benchmarkResults.length : 0;
  ...
  private generateRecommendations(suiteResults: Array<any>): string[] {
    ...
    for (const suite of suiteResults) {
      if (suite.overallScore < 70) {
        recommendations.push(
          `${suite.suiteName} performance is below threshold. Consider optimization.`
        );
      }
      ...
      for (const benchmark of suite.benchmarks) {
        if (!benchmark.passed) {
          recommendations.push(
            `${benchmark.name} failed to meet performance expectations.`
          );
        }

        if (benchmark.result.memoryStats.growth > 50) {
          recommendations.push(
            `${benchmark.name} has high memory growth. Review memory management.`
          );
        }

        if (
          benchmark.result.standardDeviation >
          benchmark.result.averageTime * 0.5
        ) {
          recommendations.push(
            `${benchmark.name} has high variance. Consider caching or optimization.`
          );
        }
      }
    }
    ...
    if (recommendations.length === 0) {
      ...
      recommendations.push(
        'Consider systematic performance review and optimization.'
      );
    }
  ...
  startContinuousMonitoring(intervalMs: number = 300000): void {
    ...
    setInterval(async () => {
      try {
        ...
        if (
          report.systemMetrics.performanceGrade === 'D' ||
          report.systemMetrics.performanceGrade === 'F'
        ) {
          console.warn(
