...
export class MessageService extends BaseService {
  ...
  async getMessage(messagePDA: Address): Promise<MessageAccount | null> {
    try {
      const account = await (this.getAccount('messageAccount') as any).fetch(
        messagePDA
      );
  ...
  async getAgentMessages(
    ...
    try {
      ...
      throw new Error(
        `Failed to fetch agent messages: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private convertMessageStatusFromProgram(
    programStatus: ProgramMessageStatus
  ): MessageStatus {
  ...
  private __messageStatusToString = (status: MessageStatus): string => {
    switch (status) {
      case MessageStatus.PENDING:
        return 'Pending';
      case MessageStatus.DELIVERED:
  ...
  private async generateMessageId(
    ...
    const hashStr = Array.from(payloadHash)
  ...
  async send(options: {
    ...
    const messageType = options.messageType
      ? this.parseMessageType(options.messageType)
      : MessageType.TEXT;
  ...
  async getFiltered(options: {
    ...
    const statusFilter = options.status
      ? this.parseMessageStatus(options.status)
      : undefined;
    const messages = await this.getAgentMessages(
      this.wallet.address,
      options.limit || 50,
      statusFilter
    );
