...
let ___createMint: any,
  ___mintTo: any,
  ___transfer: any,
  CompressedTokenProgram: any;

// Initialize compressed token functions
const __initCompressedToken = async () => {
  try {
    const compressedTokenModule = await import(
...
export class ZKCompressionService extends BaseService {
  ...
  private async initializeLightProtocol(): Promise<void> {
    try {
      ...
      console.warn(
        'Light Protocol packages not available, using fallback:',
        error
      );
      this.lightProtocol = null;
    }
  ...
  async broadcastCompressedMessage(
    ...
    if (content.length > 10000) {
      // Reasonable limit for content
      throw new Error('Content too large for compression');
    }

    if (
      messageType &&
      !['Text', 'Data', 'Command', 'Response'].includes(messageType)
    ) {
    ...
    try {
      ...
      const compressedMessage: CompressedChannelMessage = {
        channel: channelId,
        sender: (this as any).config.lightSystemProgram, // Will be set by program
        contentHash,
      ...
      if (this.config.enableBatching) {
        ...
        return new Promise((resolve, reject) => {
          const processMessage = async () => {
            try {
              ...
              if (options?.immediate) {
                ...
                try {
                  const compressionInstruction =
                    await this.createCompressionInstruction(
                      channelId,
                      compressedMessage,
                      String(wallet)
                    );
                  ...
                  console.warn(
                    'Light Protocol compression failed, using fallback:',
                    compressionError
                  );

                  const fallbackResult =
                    await this.createDeterministicCompression(
                      compressedMessage,
                      ipfsResult
                    );
                  resolve({
                ...
                if (
                  ...
                  this.batchQueue.length >= (this.config.maxBatchSize || 10)
                ) {
                  try {
                    const __batchResult = await this.processBatch(wallet);
                    const result = this.lastBatchResult || {
                      signature: await this.generateDeterministicSignature(
                        `batch_${Date.now()}`
                      ),
                      compressedAccounts: [],
                    };
              ...
              reject(
                new Error(`Failed to broadcast compressed message: ${error}`)
              );
            }
          };
        ...
        try {
          ...
          if (!signature || signature.length < 64) {
            throw new Error(
              'Invalid signature from Light Protocol compression'
            );
          }
        } catch (err) {
    ...
    try {
      ...
      const tx = await (
        program as unknown as {
          methods: {
            joinChannelCompressed: (_data: unknown[]) => {
              accounts: (_config: unknown) => {
    ...
    try {
      ...
      for (const hash of messageHashes) {
        if (
          ...
          throw new Error(
            `Invalid message hash format: ${hash}. Expected 64-character hex string.`
          );
        }
      }
      ...
      const rpcWithMethods = this.rpc as {
        getTransaction: (_sig: string, _options?: unknown) => Promise<unknown>;
      };
      const txInfo = (await rpcWithMethods.getTransaction(signature, {
        commitment: 'confirmed',
      })) as {
        compressionInfo?: {
          openedAccounts?: Array<{
            account: { hash: { toString: (_radix: number) => string } };
          }>;
        };
      } | null;
      ...
      if (
        ...
        txInfo?.compressionInfo?.openedAccounts?.[0]
      ) {
        try {
          const rpcWithProof = this.rpc as {
            getValidityProof: (
              _hash: unknown
            ) => Promise<{ root: { toString: (_radix: number) => string } }>;
          };
      ...
      return await RetryUtils.rpcCall(async () => {
        ...
        for (const account of compressedAccounts || []) {
          try {
            ...
            if (
              options.after &&
              messageData.createdAt < options.after.getTime()
            ) {
              continue;
            }
            if (
              options.before &&
              messageData.createdAt > options.before.getTime()
            ) {
              continue;
            }
  ...
  async getChannelStats(channelId: string): Promise<{
    ...
    try {
      ...
      const compressionRatio =
        uncompressedSize > 0 ? uncompressedSize / compressedSize : 1;
   ...
   */
  private async queryCompressedParticipants(
    channelId: string
  ): Promise<CompressedChannelParticipant[]> {
    try {
      const queryParams = {
        filters: [
          {
  ...
  async getMessageContent(
    ...
    try {
      ...
      const computedHash = IPFSService.createContentHash(
  ...
  private async processBatch(wallet: unknown): Promise<unknown> {
    ...
    try {
      const rpcWithProof = this.rpc as {
        getValidityProof: (_param: null) => Promise<[unknown]>;
      };
  ...
  async joinChannel(options: {
    ...
    const result = await this.joinChannelCompressed(
      options.channelPDA,
      (this.wallet as any).address ||
        (this.wallet as any).publicKey?.toString() ||
        'participant',
      this.wallet,
      options.displayName,
      options.avatar,
  ...
  private async createDeterministicCompression(
    ...
    if (typeof crypto !== 'undefined' && crypto.subtle) {
      ...
      contentHash = Array.from(hashArray, (byte) =>
  ...
  private async processEnhancedBatch(
    ...
    try {
      ...
      console.warn(
        'Light Protocol batch failed, using deterministic batch processing:',
        error
      );
    ...
    while (currentLevel.length > 1) {
      ...
      for (let i = 0; i < currentLevel.length; i += 2) {
        ...
        if (typeof crypto !== 'undefined' && crypto.subtle) {
          ...
          const hashArray = new Uint8Array(hashBuffer);
          hash = Array.from(hashArray, (byte) =>
            byte.toString(16).padStart(2, '0')
          ).join('');
        } else if (typeof process !== 'undefined' && process.versions?.node) {
          try {
  ...
  async verifyCompressedData(
    ...
    try {
      ...
      const dataHash = await this.hashMessage(
        compressedAccount.data as CompressedChannelMessage
      );
      const hashMatch =
        dataHash === originalHash || compressedAccount.hash === originalHash;
