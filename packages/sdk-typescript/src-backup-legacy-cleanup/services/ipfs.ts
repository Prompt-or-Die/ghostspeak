...
export class IPFSService extends BaseService {
    ...
    this.initPromise = (async () => {
      try {
        ...
        try {
          ...
          console.warn(
            'Warning: Native IPFS modules unavailable, IPFS features will be disabled'
          );
          throw new Error(
            `IPFS functionality requires native modules: ${nativeModuleError.message}`
          );
        }
      } catch (error) {
  ...
  static createContentHash(content: string): string {
    ...
    const keccakHash = keccak('keccak256')
      .update(
        Buffer.concat([Buffer.from(content, 'utf8'), Buffer.from([0xff])])
      )
      .digest();
