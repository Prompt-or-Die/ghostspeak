...
export class ChannelService extends BaseService {
  ...
  async getChannelsByCreator(
    ...
    try {
      ...
      return accounts
        .filter(
          (acc: { account: { creator: { equals(addr: Address): boolean } } }) =>
            acc.account.creator.equals(creator)
        )
        .slice(0, limit)
    ...
    try {
      ...
      const tx = await (
        program.methods as unknown as {
          joinChannel(): { accounts(obj: unknown): { rpc(): Promise<string> } };
        }
      )
        .joinChannel()
        .accounts({
    ...
    return retry(async () => {
      ...
      const tx = await (
        this.program.methods as unknown as {
          leaveChannel(): {
            accounts(obj: unknown): { rpc(): Promise<string> };
          };
        }
      )
        .leaveChannel()
  ...
  async inviteToChannel(
    ...
    try {
      const [invitationPDA] = await findInvitationPDA(
        channelAddress,
        inviteeAddress,
        this.programId
      );
      ...
      throw new Error(
        `Failed to invite to channel: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getChannelParticipants(channelAddress: Address): Promise<Address[]> {
    try {
      ...
      const participantAccounts = await this.getProgramAccounts(
        'participantAccount',
        [participantFilter]
      );

      // Process accounts to extract participant addresses
      const participants = await this.processAccounts(
        participantAccounts,
        'participantAccount'
      );

      return participants
        .map((participant: any) => participant.agent || participant.member)
        .filter(Boolean);
    } catch (error) {
      if (process.env.NODE_ENV === 'development') {
  ...
  async getChannelMessages(
    ...
    try {
      ...
      return accounts
        .slice(0, limit)
        .map((acc: { account: unknown }) => acc.account);
    } catch (error) {
      if (process.env.NODE_ENV === 'development') {
  ...
  ): { public: Record<string, never> } | { private: Record<string, never> } {
    switch (visibility) {
  ...
  private convertMessageType(
    messageType: string | MessageType | MessageTypeObject
  ): MessageTypeObject {
  ...
  private convertChannelAccountFromProgram(
    ...
    return {
      ...
      account: {
        ...
        creator: account.creator,
        isPublic:
          this.convertChannelVisibilityFromProgram(
            account.visibility as ChannelVisibilityObject
  ...
  async getChannelMembers(): Promise<Address[]> {
    try {
      ...
      const participantAccounts =
        await this.getProgramAccounts('participantAccount');

      // Extract member addresses from participant accounts
      const members: Address[] = [];
      const participantData = await this.processAccounts(
        participantAccounts,
        'participantAccount'
      );
      ...
      for (const participant of participantData) {
        try {
          const memberAddress =
            (participant as any).member || (participant as any).agent;
          if (memberAddress) {
  ...
  async create(options: {
    ...
    if (options.visibility === 'private') {
      result.joinCode = `join_${channelId}`;
    }
