...
export class DiscoveryService extends BaseService {
  ...
  async findAgentsByCapabilities(
    ...
    try {
      ...
      const __capabilityBitmask =
        this.convertCapabilitiesToBitmask(capabilities);
      ...
      console.log(
        `Found ${filteredAgents.length} real agents with capabilities`
      );
    ...
    try {
      ...
      const channels = await this.processAccounts<ChannelAccount>(
        ...
        (decoded, account) => ({
          ...
          maxMembers:
            ...
            decoded.maxMembers?.toNumber() ||
            0,
          memberCount:
            decoded.currentParticipants?.toNumber() ||
            decoded.memberCount?.toNumber() ||
            0,
          currentParticipants:
            decoded.currentParticipants?.toNumber() ||
            decoded.memberCount?.toNumber() ||
      ...
      const trendingChannels = await Promise.all(
        channels.map(async (channel) => {
          const activityScore = await this.calculateChannelActivity(
            channel,
            timeframeMs
          );
          return {
      ...
      const sortedChannels = trendingChannels
        ...
        .slice(0, limit)
        .map((item) =>
          this.convertChannelToDiscoveryData(item.channel, item.activityScore)
        );
    ...
    try {
      ...
      const agentsWithMetadata = await Promise.all(
        agents.map(async (agent) => {
          try {
            const metadata = await this.fetchAgentMetadata(
              agent.metadataUri,
              agent.capabilities
            );
      ...
      const matchedAgents = agentsWithMetadata
        .filter(({ agent, metadata }) => {
          ...
          if (filters) {
            ...
            if (filters.isActive !== undefined) {
              const isActive =
                agent.lastUpdated > Date.now() - 24 * 60 * 60 * 1000;
        ...
        .slice(0, limit)
        .map(({ agent, metadata }) =>
          this.convertToDiscoveryDataWithMetadata(agent, metadata)
        );

      console.log(
        `Found ${matchedAgents.length} agents matching search criteria`
      );
  ...
  async getAgentRecommendations(
    ...
    try {
      ...
      const sentMessages = await this.getProgramAccounts(
        'messageAccount',
        messageFilters,
        {
          limit: 100,
  ...
  async getNetworkStats(): Promise<{
    ...
    try {
      ...
      const [agentAccounts, channelAccounts, messageAccounts] =
        await Promise.all([
          this.getProgramAccounts('agentAccount', [], {
            useCache: true,
            cacheTtl: 300000,
          }), // 5 min cache
          this.getProgramAccounts('channelAccount', [], {
            useCache: true,
            cacheTtl: 300000,
          }),
          this.getProgramAccounts('messageAccount', [], {
            limit: 1000,
            useCache: true,
            cacheTtl: 60000,
          }),
      ...
      const activeAgents = agents.filter(
        (agent) => agent.lastUpdated > twentyFourHoursAgo
      ).length;
  ...
  async searchMessages(
    ...
    try {
      ...
      for (const account of accounts) {
        try {
          const decoded = this.ensureInitialized().coder.accounts.decode(
            'messageAccount',
  ...
  async searchChannels(
    ...
    try {
      ...
      let channels: ChannelAccount[] =
        await this.processAccounts<ChannelAccount>(
          accounts,
          'channelAccount',
          (decoded, account) => ({
            pubkey: address(account.pubkey),
  ...
  private async calculateChannelActivity(
  ...
  private async fetchAgentMetadata(
    ...
    const agentCapabilities =
      capabilities !== undefined
        ? capabilities
        : this.generateDeterministicCapabilities(agentAddress);
  ...
  private calculateRecommendationScore(
    ...
    const hoursSinceActive =
      (Date.now() - agent.lastUpdated) / (1000 * 60 * 60);
  ...
  private convertChannelVisibilityFromProgram(
    ...
    if (programVisibility.restricted !== undefined)
      return ChannelVisibility.Restricted;
  ...
  private applyAgentFilters(
    ...
    return agents.filter((agent) => {
      ...
      if (
        ...
        agent.lastUpdated < filters.lastActiveAfter
      ) {
        return false;
      }
      if (
        filters.lastActiveBefore &&
        agent.lastUpdated > filters.lastActiveBefore
      ) {
        return false;
      }
  ...
  private applyMessageFilters(
    ...
    return messages.filter((msg) => {
      ...
      if (filters.messageType) {
        const messageTypes = Array.isArray(filters.messageType)
          ? filters.messageType
          : [filters.messageType];
  ...
  private applyChannelFilters(
    ...
    return channels.filter((channel) => {
      ...
      if (
        ...
        !channel.name.toLowerCase().includes(filters.nameContains.toLowerCase())
      )
        return false;
      if (
