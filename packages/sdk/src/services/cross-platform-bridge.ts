...
export class CrossPlatformBridgeService {
  ...
  async registerPlatform(config: IPlatformConfig): Promise<{
    ...
    try {
      logger.bridge.info(`🔌 Registering platform: ${config.platform}`);
    ...
    try {
      ...
      for (const registration of platformRegistrations) {
        try {
          ...
          logger.bridge.info(`✅ Agent registered on ${registration.platform}`);
        } catch (error) {
  ...
  async discoverCrossPlatformAgents(
    ...
    try {
      ...
      logger.bridge.info('✅ Cross-platform agent discovery complete:', {
