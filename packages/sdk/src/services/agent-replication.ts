...
export class AgentReplicationService {
  ...
  async createReplicationTemplate(
    ...
    try {
      ...
      logger.agent.info('✅ Replication template created:', signature);
      ...
      logger.agent.error('❌ Failed to create replication template:', error);
      throw new Error(
  ...
  async replicateAgent(
    ...
    logger.agent.info('Replicating agent from template:', templatePda);
    if (customizations && Object.keys(customizations).length > 0) {
      logger.agent.info('Customizations:', customizations);
    }
    ...
    try {
      ...
      logger.agent.info('✅ Agent replicated successfully:', signature);
      ...
      logger.agent.error('❌ Failed to replicate agent:', error);
      throw new Error(
  ...
  async getReplicationTemplate(
    ...
    try {
      ...
      logger.agent.error('❌ Failed to get replication template:', error);
      return null;
    }
  }
  ...
  async listCreatorTemplates(
    ...
    try {
      ...
      return accounts.map(account =>
        this.parseReplicationTemplate(account.account.data)
      );
    } catch (error) {
      logger.agent.error('❌ Failed to list creator templates:', error);
      return [];
    }
  }
  ...
  async getReplicationHistory(
    ...
    try {
      ...
      logger.agent.error('❌ Failed to get replication history:', error);
      return [];
    }
  }
   ...
   */
  private async getReplicatedAgentPda(
    buyer: Address,
    agentId: string
  ): Promise<Address> {
   ...
   */
  private parseReplicationTemplate(
    _data: string | Uint8Array
  ): IReplicationTemplate {
    return {
      id: 'mock_template_id',
