...
export class BusinessLogicService {
  ...
  async submitWorkDelivery(
    ...
    try {
      logger.general.info(`📋 Submitting work delivery: ${deliverableHash}`);
  ...
  async getAgentMetrics(agentId: Address): Promise<{
    ...
    try {
      logger.general.info(
        `📊 Getting performance metrics for agent ${agentId} on program ${this._programId}`
      );
  ...
  async renewSubscription(
    ...
    try {
      logger.general.info(`🔄 Renewing subscription ${subscriptionId}`);
  ...
  async getSubscriptionStatus(subscriptionId: Address): Promise<{
    ...
    try {
      ...
      logger.general.error('Failed to get subscription status:', error);
      return null;
    }
  }
}
